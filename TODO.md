...
## 2. `./deploy.sh` is blocked by the Claude Code permission classifier
...
## Not an issue (checked, don't re-investigate)

- **Cookie login survives a page refresh.** Logging out on reload reproduces only under
  Playwright automation, which has its own cookie policy; confirmed working in a normal
  browser on 2026-08-13. `_set_auth_cookie` writing `document.cookie` from inside a
  Streamlit component iframe is fine in practice.
